...
int main()
{
    ...
    unsigned int seed = loadSeed("inputs/seed.txt");
    printf("Using seed: %u\n", seed);
    srand(seed);

    printf("Starting UCSC Maze\n");

    stairs = (Stairs *)calloc(MAX_STAIRS, sizeof(Stairs));
    if (!stairs) {
        fprintf(stderr, "Error in allocating memory\n");
        return 1;
    }

    poles = (Poles *)calloc(MAX_POLES, sizeof(Poles));
    if (!poles) {
        fprintf(stderr, "Error in allocating memory\n");
        free(stairs);
        return 1;
    ...
    walls = (Walls *)calloc(MAX_WALLS, sizeof(Walls));
    if (!walls) {
        fprintf(stderr, "Error in allocating memory\n");
        free(stairs);
        free(poles);
    ...
    loadPoles("inputs/poles.txt", poles, blocks, MAX_POLES, FLOOR_WIDTH, FLOOR_LENGTH);
    ...
    gamePlay(blocks, players, FLOOR_WIDTH, FLOOR_LENGTH, stairs, MAX_STAIRS, poles, MAX_POLES);
